...
- **Astronomische Nächte:** Nur Zeiträume, in denen der Himmel vollständig dunkel ist (`astronomical_night == 1`).
- **Himmelstemperatur:** Nur Nächte mit Temperaturen unter 0°C (`sky_temperature < 0`).

---

**Berechnungen**

1. **Maximale Helligkeit pro Nacht**:  
   Für jede Nacht (`night_id`) wird der höchste gemessene MSAS-Wert berechnet:  
   `max_msas = max(msas)`.

2. **Prozent der Zeit mit Temperaturen unter 0°C**:  
   Der Anteil der Zeit in einer Nacht, in der die Himmelstemperatur < 0°C lag:  
   `sky_temp_below_zero_pct = (Summe(sky_temperature < 0) ÷ Anzahl der Messungen) × 100`.

3. **Anzahl der Nächte mit MSAS > 21.3**:  
   Nächte, bei denen `max_msas > 21.3`, werden gezählt:  
   `nights_over_21_3 = Summe(max_msas > 21.3)`.

4. **Anzahl der kalten Nächte**:  
   Nächte, in denen mindestens 90% der Zeit die Himmelstemperatur unter 0°C lag:  
   `nights_cold = Summe(sky_temp_below_zero_pct ≥ 90)`.

---

#### Zweiter Plot: Verhältnis zwischen Nächten mit hoher Himmelsqualität und kalten Nächten

**Beschreibung**:  
Dieser Plot zeigt den **prozentualen Anteil** der Nächte, bei denen `max_msas > 21.3`, im Vergleich zu den kalten Nächten.

- **Darstellung**: Pro Jahr wird ein kurzer Strich gezeichnet, der den Anteil (%) zeigt.
- **Trendlinie**: Eine gestrichelte Linie zeigt ab mindestens 3 Jahren einen langfristigen Trend (lineare Regression).

---

**Berechnung des Verhältnisses**

Das Verhältnis (%) zwischen Nächten mit `max_msas > 21.3` und kalten Nächten wird berechnet als:
- **Wenn kalte Nächte vorhanden sind (`nights_cold > 0`)**:  
  `ratio = (nights_over_21_3 ÷ nights_cold) × 100`.

- **Wenn keine kalten Nächte vorhanden sind**:  
  `ratio = 0`.
...
1. **Erster Plot**: Zeigt die jährliche Anzahl von Nächten in zwei Kategorien:  
   - Nächte mit hoher Himmelsqualität (max. MSAS > 21.3).  
   - Nächte, in denen der Himmel überwiegend kalt war (Temperatur < 0°C).

2. **Zweiter Plot**: Visualisiert den Anteil der Nächte mit hoher Himmelsqualität im Vergleich zu den kalten Nächten.

3. **Filterbedingungen**:  
   - Nur astronomische Nächte werden berücksichtigt.  
   - Himmelstemperatur muss unter 0°C liegen.  
   - Die Daten sind pro Jahr aggregiert und nach Bedingungen gefiltert.

**Diagrammziel**: Veranschaulichen, wie oft besonders dunkle Nächte auftreten und wie sie mit kalten Nächten in Beziehung stehen.
